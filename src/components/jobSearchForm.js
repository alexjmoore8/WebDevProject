...
import './loginRegister/css/jobSearchResults.css'; // This is where you import your CSS file
...
export function JobSearchForm() {
    ...
    const redirectToRecommendedJobs = () => {
        navigate('/recommendedJobs');     //Change route here for recommended jobs
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (!state || !city || !salary) {
            setJobs([]); // Clear any existing jobs
            setShowButton(false); // Hide the button
            setSuccessMessage(''); // Clear success message
            setMessage("State, City, and Salary can't be empty.");
            return; // Stop the function execution if fields are empty
        } else {
            setMessage(''); // Reset error message if fields are filled
        }
    ...
    return (
        <div className="container">
            <div className="form-box">
                ...
                {/* Conditional button based on search results */}
                {showButton && (
